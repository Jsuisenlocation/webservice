...
def insert():
    if request.method == "POST":
        ...
        cursor = mysql.connection.cursor()
        cursor.execute("INSERT INTO reservation (reservation.index, User, Nom , Date , ID ,depart_reserv , arrivee_reserv ,Quantite, Prix ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",(Idxmax, user_id, Nom,date,trainid,départ,arrivée,quantité, prix_tot))
        mysql.connection.commit()
...
def update():
    if request.method == "POST":
        ...
        cursor = mysql.connection.cursor()
        cursor.execute("UPDATE reservation SET Nom=%s, Date=%s, ID=%s, depart_reserv=%s, arrivee_reserv=%s, Quantite=%s WHERE reservation.index=%s ",(Nom,date,trainid,départ,arrivée,quantité,orderID))
...
def delete(id):
    ...
    cursor = mysql.connection.cursor()
    cursor.execute("DELETE FROM reservation WHERE reservation.index =%s", (id,))
    mysql.connection.commit()
    return redirect(url_for('Acceuil'))
...
def update_selectDepart():
    ...
    ################################## 
    return jsonify(arrivé_option = arrivé_html) #, train_selected = trainId_html, amountAllow = amount_html) #
...
def update_selectArrive():
    ...
    ##################################
    # 2. requête base de données
    sqlCondition = "Date = '"+select_date+"' AND Depart = '"+select_depart+"' AND Destination = '"+select_arrivée+"'"
...
def update_seleTrainId():
    ...
    select_quantite = int(select_quantite) if select_quantite is not None else 1
    # ########## 2. Requête pour les places ##########
    sqlCondi1 = "Date = '"+select_date+"' AND Depart = '"+select_depart+"' AND Destination = '"+select_arrivée+"' AND ID='"+select_trainid+"'"
    place_restante = valeur_db("Places", "train", sqlCondi1)[0]
    sqlCondi2 = "Date = '"+select_date+"' AND ID ='"+select_trainid+"' AND Depart_reserv = '"+select_depart+"' AND arrivee_reserv ='"+select_arrivée+"'"
    cursor = mysql.connection.cursor()
    ...
    #########################################
    return jsonify(quantite_limit ="{}".format(quantite_max), affiche_prix="{}".format(prix))
